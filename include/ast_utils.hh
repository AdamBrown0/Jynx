...
public:
    ...
    static std::string node_to_string(ASTNode<Extra>* node) {
        ...
        if (auto literal = dynamic_cast<LiteralExprNode<Extra>*>(node)) {
            ...
            return "(" + node_to_string(binary->left.get()) + " " +
                   binary->op.getValue() + " " +
                   node_to_string(binary->right.get()) + ")";
        }
        ...
        // ============ Statement Nodes ============
            ...
            return "Program{" + std::to_string(program->children.size()) + " statements}";
        }
        else if (auto block = dynamic_cast<BlockNode<Extra>*>(node)) {
            return "Block{" + std::to_string(block->statements.size()) + " statements}";
        }
        else if (auto if_stmt = dynamic_cast<IfStmtNode<Extra>*>(node)) {
            std::string result = "if (" + node_to_string(if_stmt->condition.get()) + ")";
        ...
        else if (auto constructor_decl = dynamic_cast<ConstructorDeclNode<Extra>*>(node)) {
    ...
    static std::string node_type_name(ASTNode<Extra>* node) {
        ...
        if (dynamic_cast<ConstructorDeclNode<Extra>*>(node)) return "ConstructorDecl";
